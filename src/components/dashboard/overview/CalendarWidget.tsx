    ...
    <div className={`bg-indigo-50/30 dark:bg-indigo-950/10 backdrop-blur-2xl px-5 md:p-8 rounded-none md:rounded-[2.5rem] border-b md:border border-indigo-500/10 dark:border-indigo-500/5 flex flex-col overflow-hidden relative group transition-all duration-300 ${isExpanded ? 'h-full py-5' : 'h-auto md:h-full py-3 md:py-8'}`}>
      ...
      >
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 md:w-10 md:h-10 rounded-2xl bg-indigo-100 dark:bg-indigo-900/30 flex items-center justify-center border border-indigo-500/20">
          ...
          <div className="flex flex-col items-start text-left">
            ...
            {!isExpanded && (
              <span className="text-[8px] text-zinc-500 font-black uppercase tracking-widest md:hidden opacity-50">Dnešné plány a udalosti</span>
            )}
          </div>
        </div>
        <div className="flex items-center gap-3 md:hidden">
          ...
          <ChevronDown className={`w-5 h-5 transition-all duration-300 ${isExpanded ? 'rotate-180 text-foreground' : 'text-zinc-400'}`} />
        </div>
      ...
      <div className={`flex-1 transition-all duration-500 ${isExpanded ? 'mt-6 opacity-100 block' : 'hidden md:block md:mt-6 opacity-0 md:opacity-100'}`}>
        <div className="md:hidden flex items-center justify-between mb-6 bg-white/30 dark:bg-zinc-800/20 p-2 rounded-2xl border border-black/5">
          <button onClick={() => changeWeek(-1)} className="p-2"><ChevronLeft className="w-4 h-4" /></button>
          <span className="text-[10px] font-black uppercase">{currentWeekNumber}. Týždeň</span>
          <button onClick={() => changeWeek(1)} className="p-2"><ChevronRight className="w-4 h-4" /></button>
        </div>
