...
export function ToolCard({ toolId, hasAccess }: { toolId: string; hasAccess: boolean }) {
  ...
  return (
    <div className="border border-gray-200 bg-white rounded-xl p-6 shadow-sm hover:shadow-md transition-shadow">
      ...
      <p className="text-gray-600 mb-6 h-12 overflow-hidden">{tool.description}</p>

      {hasAccess ? (
        <Link
          href={tool.path}
          className="block w-full text-center bg-gray-900 text-white py-2.5 px-4 rounded-lg hover:bg-gray-800 transition-colors font-medium"
        >
          Open Tool
        </Link>
      ) : (
