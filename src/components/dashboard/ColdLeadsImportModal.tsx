...
export function ColdLeadsImportModal({
  ...
  const [autoStartEnrichment, setAutoStartEnrichment] = useState(true);
    ...
    try {
      ...
      if (res.success && res.items) {
        if (res.duplicates && res.duplicates > 0) {
            toast.warning(`Preskočených ${res.duplicates} duplikátov.`);
        }
        ...
        <div className="px-8 py-6 border-b border-gray-100 flex items-center justify-between bg-gray-50/50">
          <div>
            ...
            <div className="flex gap-2 mt-1">
              ...
              <div className={`h-1 w-8 rounded-full ${step === "map" ? "bg-blue-600" : "bg-gray-200"}`} />
            </div>
          </div>
          {!isUploading && (
