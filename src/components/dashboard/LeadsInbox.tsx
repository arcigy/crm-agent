  ...
  const handleSaveContact = async (e: React.MouseEvent, msg: GmailMessage) => {
    ...
    if (!phone) {
      ...
      const uniquePhones = phones
        ? Array.from(new Set(phones.map((p) => p.trim())))
        : [];
    ...
    if (!website || website === "—") {
      ...
      const websiteRegex = new RegExp(
        `(?:https?:\\/\\/)?(?:www\\.)?([a-zA-Z0-9-]+\\.(?:${tldPattern}))(?:\\/[^\\s]*)?`,
        "gi",
      );
      ...
      const textWithoutEmails = textToSearch.replace(
      ...
      if (data.success && data.plan?.actions) {
        ...
        for (const action of data.plan.actions) {
          ...
          if (action.tool === "draft_reply") {
            ...
            alert(
              result.success
                ? `✅ Contact Created: ${action.parameters.name}`
            ...
            alert(
              result.success
                ? `✅ Deal Created: ${action.parameters.name}`
                : `❌ Deal Error: ${result.error}`,
            );
          } else if (action.tool === "check_availability") {
            const result = await agentCheckAvailability(
              action.parameters.time_range,
            );
            if (result.success) {
            ...
            if (
              ...
              ["all", "unread", "leads", "sms", "calls"].includes(
                action.parameters.tab,
              )
            ) {
    ...
    <div className="flex h-full gap-0 bg-background rounded-none relative transition-colors duration-300">
      ...
      <div className="w-64 flex flex-col gap-1 border-r border-border bg-sidebar p-4 transition-colors">
        ...
        <button
          ...
          className={`flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-bold transition-all text-left ${selectedTab === "all" ? "bg-blue-600 text-white shadow-xl" : "text-foreground/70 hover:bg-card hover:text-foreground hover:shadow-sm"}`}
        >
        ...
        <div className="space-y-1 mt-2">
          <button
            ...
            className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-bold transition-all text-left ${selectedTab === "sms" ? "bg-blue-600 text-white shadow-xl" : "text-foreground/70 hover:bg-card hover:text-foreground hover:shadow-sm"}`}
          >
            <MessageSquare className="w-4 h-4" /> SMS
          </button>
          <button
            onClick={() => setSelectedTab("calls")}
            className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-bold transition-all text-left ${selectedTab === "calls" ? "bg-blue-600 text-white shadow-xl" : "text-foreground/70 hover:bg-card hover:text-foreground hover:shadow-sm"}`}
          >
        ...
        <button
          ...
          className={`flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-bold transition-all text-left ${selectedTab === "unread" ? "bg-blue-600 text-white shadow-xl" : "text-foreground/70 hover:bg-card hover:text-foreground hover:shadow-sm"}`}
        >
        ...
        <div className="px-6 py-4 border-b border-border flex items-center justify-between bg-card/30">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className={`p-2.5 rounded-xl bg-card text-foreground/70 border border-border hover:bg-muted transition-all ${loading ? "animate-spin" : ""}`}
            >
            ...
            <div className="divide-y divide-border">
              {allItems.map((item: any) => {
                if (item.itemType === "email") {
                        ...
                        <div className="flex-1 min-w-0 grid grid-cols-12 gap-4 items-center">
                          <div className="col-span-2">
                            ...
                            <span className="text-[10px] uppercase font-bold text-gray-500 tracking-wider">
                              {format(new Date(msg.date), "d. MMM HH:mm", {
                                locale: sk,
                              })}
                            </span>
                          </div>
                            ...
                            <div className="flex flex-col lg:flex-row gap-6 items-stretch">
                              ...
                              <div className="flex-1 bg-white/60 backdrop-blur-sm rounded-2xl p-5 border border-white/50 shadow-sm flex flex-col justify-between">
                                ...
                                <div className="mt-4 pt-4 border-t border-gray-100/50 flex items-center gap-4">
                                  ...
                                  >
                                    <UserPlus className="w-3.5 h-3.5" /> Uložiť
                                    kontakt
                                  </button>
                  ...
                  const isMissed =
                    log.direction === "missed" || log.direction === "rejected";
