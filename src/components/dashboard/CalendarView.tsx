...
import { getCalendarEvents, getCalendarConnectionStatus, disconnectGoogle } from '@/app/actions/calendar';
...
export function CalendarView() {
    ...
    const getEventsForDay = (day: Date) => {
        return events.filter(event => {
            const eventStart = event.start.dateTime ? parseISO(event.start.dateTime) : (event.start.date ? parseISO(event.start.date) : null);
    ...
    if (loading) {
        return (
            <div className="flex flex-col items-center justify-center h-[500px] bg-white/50 backdrop-blur-sm rounded-3xl border border-white/20 shadow-xl">
                ...
                <p className="text-gray-500 font-medium tracking-tight">Synchronizing with Google...</p>
            </div>
        );
    }
    ...
    if (isConnected === false) {
        return (
            <div className="flex flex-col items-center justify-center py-24 bg-white rounded-[2.5rem] shadow-2xl border border-gray-100 overflow-hidden relative">
                ...
                <h2 className="text-4xl font-black text-gray-900 mb-3 tracking-tight">Connect Workspace</h2>
                <p className="text-gray-500 max-w-md text-center mb-10 px-8 text-lg font-medium leading-relaxed">
                    Sync your Google Calendar to manage meetings, view your schedule, and close deals directly from the CRM.
                </p>
                <div className="flex flex-col gap-4 w-full max-w-xs">
                    ...
                    <p className="text-[10px] text-gray-400 text-center uppercase font-black tracking-widest">Secure OAuth2 Authorization</p>
                </div>
            </div>
        );
    }
            ...
            <header className="px-8 py-6 border-b border-gray-100 flex items-center justify-between bg-white/80 backdrop-blur-xl sticky top-0 z-20">
                <div className="flex items-center gap-8">
                    <div className="flex flex-col">
                        <span className="text-[10px] font-black text-blue-600 uppercase tracking-[0.2em] mb-1">Calendar</span>
                ...
                <div className="flex items-center gap-4">
                    {syncing && (
                        <div className="flex items-center gap-2 bg-blue-50 px-3 py-1.5 rounded-full border border-blue-100 flex-none">
                            ...
                            <span className="text-[10px] font-black text-blue-600 uppercase tracking-wider">Syncing</span>
                        </div>
                    )}
                    <button
                    ...
                    <button className="bg-gray-900 hover:bg-black text-white px-6 py-3.5 rounded-2xl font-bold text-sm flex items-center gap-2 transition-all shadow-xl active:scale-95">
            ...
            <div className="flex-1 grid grid-cols-1 lg:grid-cols-5 overflow-hidden">
                ...
                <div className="lg:col-span-1 border-r border-gray-100 bg-gray-50/20 overflow-y-auto p-6 space-y-6">
                    <div className="flex items-center justify-between">
                        <h3 className="text-xs font-black uppercase tracking-[0.2em] text-gray-400">Upcoming</h3>
                        <span className="bg-gray-200/50 text-gray-500 text-[10px] font-black px-2 py-0.5 rounded-full">{events.length}</span>
                    </div>
                    ...
                    {events.length === 0 ? (
                        <div className="p-10 text-center bg-white rounded-3xl border border-dashed border-gray-200 shadow-sm">
                            ...
                            <p className="text-sm text-gray-400 font-bold tracking-tight">No Events Scheduled</p>
                        </div>
                    ) : (
                        <div className="space-y-3">
                            {events.slice(0, 10).map((event) => {
                                const startTime = event.start.dateTime ? parseISO(event.start.dateTime) : (event.start.date ? parseISO(event.start.date) : null);
                                return (
                                    <div key={event.id} className="bg-white p-4 rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition-all cursor-pointer group hover:-translate-y-1">
                                        ...
                                        <div className="space-y-1">
                                            {startTime && (
                                                <div className="flex items-center gap-2 text-[10px] text-gray-500 font-black uppercase tracking-wider">
                                                    ...
                                                    {format(startTime, 'HH:mm')} • {format(startTime, 'MMM d')}
                                                </div>
                                            )}
                                            {event.hangoutLink && (
                        ...
                        {calendarDays.map((day, idx) => {
                            ...
                            return (
                                <div
                                    ...
                                    `}
                                >
                                    <span className={`text-xs font-black self-end w-7 h-7 flex items-center justify-center rounded-full transition-all
                                    ...
                                    <div className="flex flex-col gap-1 overflow-y-auto max-h-[100px] scrollbar-hide">
                                        {dayEvents.map(event => (
                                            <div
                                                ...
                                                title={event.summary}
                                            >
                                                <p className="text-[9px] font-bold text-blue-900 truncate tracking-tight">{event.summary}</p>
                                            </div>
                                        ))}
                                    </div>

                                    <button className="absolute bottom-2 left-2 p-1 rounded-md text-gray-200 hover:text-gray-400 opacity-0 group-hover:opacity-100 transition-all">
