  ...
  return (
    <div className="flex h-screen w-full bg-slate-50 dark:bg-slate-950 overflow-hidden transition-colors duration-300 relative">
      ...
      >
        ...
        <div className="flex-1 flex flex-col justify-center py-4 px-3 overflow-y-auto scrollbar-hide space-y-1">
          {menuItems
            ...
            <Link 
              ...
              href={item.href}
              className={`
                flex items-center gap-3 px-4 py-3.5 rounded-xl text-[15px] font-semibold tracking-wide transition-all duration-300
                ${pathname === item.href 
              ...
              `}
            >
              <item.icon className={`w-5 h-5 transition-opacity duration-300 ${pathname === item.href ? "opacity-100 text-indigo-400" : "opacity-60 group-hover:opacity-80"}`} />
              <span>{item.name}</span>
            </Link>
          ))}
        </div>
        ...
        <div className="p-4 border-t border-white/5 bg-black/20 space-y-2">
          ...
          <Link
            ...
            className="flex items-center gap-3 rounded-xl px-4 py-3 text-[15px] font-semibold tracking-wide text-zinc-500 hover:bg-zinc-800/40 hover:text-zinc-200 transition-all group"
          >
