...
function getMessageBody(payload: any): { text: string; html: string; attachments: any[] } {
    let text = '';
    let html = '';
...
export async function GET() {
    try {
        ...
        if (!listRes.data.messages) return NextResponse.json({ isConnected: true, messages: [] });
        ...
        const messages = await Promise.all(
            listRes.data.messages.map(async (m) => {
                try {
                    ...
                    const { text, html, attachments } = getMessageBody(detail.data.payload);
