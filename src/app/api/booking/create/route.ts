...
export async function POST(req: Request) {
    try {
        ...
        // @ts-ignore
        const crmUsers = await directus.request(readItems('crm_users', {
            filter: { first_name: { _icontains: username } }, // Simple match for now
            limit: 1
        }));

        const crmUser = crmUsers?.[0];
        if (!crmUser) return NextResponse.json({ error: 'User not found' }, { status: 404 });

        // 2. Identify/Create Contact in CRM
        // @ts-ignore
        let contact = (await directus.request(readItems('contacts', {
        ...
        if (!contact) {
            ...
            // @ts-ignore
            contact = await directus.request(createItem('contacts', {
                first_name: fName,
        ...
        const clerkUsers = await client.users.getUserList({ emailAddress: [crmUser.email] });
        const clerkUser = clerkUsers.data[0];

        if (clerkUser) {
            const tokenRes = await client.users.getUserOauthAccessToken(clerkUser.id, 'oauth_google');
