...
export async function getSmartLeadsStats() {
    try {
        const userEmail = await getUserEmail();
        if (!userEmail) throw new Error("Unauthorized");
        ...
        // Sum up leads from all campaigns
        // Structure usually has 'lead_count' or 'total_leads'
        let totalActiveLeads = 0;
        let campaignsCount = 0;

        if (Array.isArray(campaigns)) {
            campaignsCount = campaigns.length;
            totalActiveLeads = campaigns.reduce((sum: number, c: any) => {
                // Try different common property names
                const count = c.lead_count || c.total_leads || c.leads_count || 0;
                return sum + count;
            }, 0);
        }
        ...
        return { 
            success: true, 
            data: {
                active_leads: totalActiveLeads,
                limit: 2000,
                campaigns_count: campaignsCount
            }
        };
    } catch (e: any) {
         console.error("Failed to get SmartLeads stats:", e);
         return { success: false, error: e.message };
    }
}

export async function cleanupSmartLeadsCampaign(campaignId: string) {
    try {
        const userEmail = await getUserEmail();
        if (!userEmail) throw new Error("Unauthorized");

        const { getCampaignLeads, deleteLeadFromCampaign } = await import("@/lib/smartleads");
        
        // 1. Fetch leads from campaign
        const leads = await getCampaignLeads(campaignId); 
        
        if (!leads || !Array.isArray(leads)) {
            return { success: false, error: "Invalid response from SmartLeads" };
        }
        ...
        // 2. Filter and Delete
        // Criteria: Finished sequence AND No reply
        for (const lead of leads) {
            examinedCount++;

            // Status normalization
            const status = (lead.status || "").toUpperCase();
            const category = lead.lead_category_id; // 5 = Completed, 3 = Replied
            
            // Check if Completed
            // COMPLETED means sequence finished.
            const isCompleted = status === "COMPLETED" || category === 5;
            
            // Check if Replied
            // We want to KEEP replied leads usually? Or delete them too? 
            // User said "remove those who didn't reply after all followups".
            // So we KEEP replied leads (as they are valuable).
            
            const hasReplied = lead.stats?.replied || lead.replied || status === "REPLIED" || category === 3;

            if (isCompleted && !hasReplied) {
                // DELETE from SmartLeads
                await deleteLeadFromCampaign(campaignId, lead.id);
                // We don't await the result to speed up? actually we should to avoid rate limits
                // But one by one is slow. SmartLeads might have bulk delete.
                deletedCount++;
            }
        }

        revalidatePath("/dashboard/cold-outreach");
        return { success: true, count: deletedCount, examined: examinedCount };

    } catch (e: any) {
        console.error("Cleanup SmartLeads failed:", e);
        return { success: false, error: e.message || String(e) };
    }
}
